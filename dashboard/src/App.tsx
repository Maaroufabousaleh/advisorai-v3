...
function DataWorkspace({ data }: { data: DashboardOverview }) {
  return <div className="workspace-stack"><div className="data-health-strip"><div><Database size={20} /><strong>Point-in-time data spine</strong><span>Snapshots carry first-available timestamps, revision lineage, parser versions, and source grades.</span></div><div className="health-number"><strong>{data.status.data_freshness_seconds}s</strong><span>freshest market tick</span></div></div><section className="panel wide-table"><div className="panel-heading"><div><div className="section-kicker"><Database size={14} /> SOURCE HEALTH</div><h2>Quality monitor</h2></div><span className="panel-heading-meta">AS OF {formatTime(data.as_of)} UTC</span></div><div className="data-table quality-table"><div className="data-table-row table-head"><span>DATASET</span><span>STATE</span><span>FRESHNESS</span><span>OBSERVATIONS</span><span>SOURCE FAMILIES</span><span>FINDING</span></div>{data.data_quality.map((row) => <div className="data-table-row" key={row.dataset}><span><strong>{row.dataset}</strong><small>immutable projection</small></span><span><StatusPill tone={row.tone} label={row.state} /></span><span className="mono">{row.freshness}</span><span className="mono">{row.observations}</span><span>{row.source_families}</span><span className="muted">{row.finding}</span></div>)}</div></section><SourceHealthPanel sources={data.source_health ?? []} /><div className="model-lanes"><ModelLane title="Forecast candidates" icon={<LineChart size={16} />} detail="Naive · LightGBM · Chronos-2" state="baseline included" /><ModelLane title="Evidence routes" icon={<BookOpenCheck size={16} />} detail="2 source families minimum" state="gate enforced" /><ModelLane title="Artifact lineage" icon={<Boxes size={16} />} detail="Bronze → Silver → Gold" state="hash bound" /></div></div>
}

function sourceHealthTone(source: SourceHealthView): Tone {
  if (source.state === 'HEALTHY' && !source.fail_closed) return 'positive'
  if (source.state === 'QUARANTINED' || source.state === 'DISCONNECTED') return 'critical'
  if (source.state === 'STALE' || source.state === 'DEGRADED' || source.state === 'RECOVERING') return 'warning'
  return 'neutral'
}

function sourceAge(source: SourceHealthView): string {
  return source.last_event_age_seconds === null ? '—' : `${source.last_event_age_seconds.toFixed(1)}s`
}

function SourceHealthPanel({ sources }: { sources: SourceHealthView[] }) {
  return <section className="panel wide-table source-health-panel"><div className="panel-heading"><div><div className="section-kicker"><Wifi size={14} /> PUBLIC SOURCE HEALTH</div><h2>Actual provider state</h2></div><div className="source-health-heading-meta"><span className="locked-label">READ ONLY</span><span className="panel-heading-meta">{sources.length} LINES</span></div></div><div className="source-health-note"><Activity size={14} /><span>Sanitized operational projection. Provider identity is explicit; failover never implies historical continuity.</span></div>{sources.length === 0 ? <div className="source-health-empty" role="status"><span className="signal-dot tone-warning" /><span>No live source-health snapshot is available. Synthetic dashboard state is not an admission record.</span></div> : <div className="data-table source-health-table" role="table" aria-label="Public market data source health"><div className="source-health-row source-health-head" role="row"><span>SOURCE / PROVIDER</span><span>ASSET</span><span>STATE</span><span>AGE</span><span>RECONNECTS</span><span>GAPS</span><span>DISAGREEMENT</span><span>SNAPSHOT</span><span>DECISION</span></div>{sources.map((source) => <div className="source-health-row" role="row" key={`${source.source_id}:${source.symbol}`}><span><strong>{source.source_id}</strong><small>{source.actual_provider_identity}</small></span><span className="mono">{source.symbol}</span><span><StatusPill tone={sourceHealthTone(source)} label={source.state} /></span><span className="mono">{sourceAge(source)}</span><span className="mono">{source.reconnect_count}</span><span className="mono">{source.sequence_gap_count}</span><span className="mono">{source.disagreement_state}</span><span className="mono">{source.snapshot_recovery_state}</span><span className={`source-decision ${source.fail_closed ? 'tone-warning' : 'tone-positive'}`}>{source.fail_closed ? 'FAIL-CLOSED' : 'ELIGIBLE'}</span></div>)}</div>}</section>
}
