"""Opt-in, real read-only smoke for Binance Spot Testnet.

The runner resolves only the ``PAPER_VENUE`` scope, verifies provider product
truth for BTCUSDT and ETHUSDT, and then performs authenticated account,
balance, position, open-order, and fill reads.  It never submits, cancels,
transfers, or withdraws.  Persisted output contains aggregate/schema metadata
only; account values and identifiers are deliberately omitted.
"""
...
def _scoped_configuration_hash(resolver: CredentialResolver) -> str:
    ...
    card = ConnectorCard(
        name="binance-spot-testnet-transition-config",
        owner="operator",
        purpose="BTC/ETH paper-testnet execution transition",
        ...
        credential_refs=tuple(name for name in _CREDENTIAL_REFS if name in scoped),
...
def _write_evidence(payload: dict[str, object], evidence_dir: Path) -> dict[str, object]:
    ...
    (evidence_dir / "latest.json").write_text(
        json.dumps(
            {
                "schema": "advisorai.phase2.binance-spot-testnet.read-only-smoke.latest.v1",
                "run_id": run_id,
...
def main() -> int:
    ...
    parser.add_argument(
        "--configuration-hash",
        required=True,
        help="hash emitted by the prior zero-network configuration check; not a secret",
    )
    ...
    passed = len(operations) == len(checks) + 3 and all(
        record["status"] == "ok" for record in operations
    )
    base.update(
        {
            "status": "passed" if passed else "failed",
            "reason": "read_only_smoke_passed" if passed else "read_only_operation_failed",
            "operations": operations,
            "network_calls": transport.client.request_count,
        }
    )
