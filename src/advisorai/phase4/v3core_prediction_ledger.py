...
class ForwardPredictionLedgerEntry(BaseModel):
    ...
    def validate_entry(self) -> ForwardPredictionLedgerEntry:
        ...
        if _hash_payload(unsigned) != self.record_hash:
            # Optional candidate identity fields were added after the first
            # ledger roots existed. Accept their exact legacy serialization so
            # a schema extension does not invalidate preserved append-only
            # evidence; newly appended entries always use the full payload.
