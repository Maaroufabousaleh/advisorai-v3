...
| 4 | Naive/statistical/LightGBM boundary, installed CPU LightGBM/Transformers extras with typed checkpoint-bound runner paths, FinBERT-family typed news challenger, calibration/abstention, optional model quarantine, GPU lease, and evidence-bound model lifecycle promotion | `tests/models` | Deterministic fixtures pass; named checkpoint/resource/net-utility bake-offs remain pending |
