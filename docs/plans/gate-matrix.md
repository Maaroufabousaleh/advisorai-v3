...
| Phase 3 V3-Core source spine | phase-03 plan; real-api-paper-transition.md | yes, including bounded raw-first REST/WSS qualification runners and native event-time normalization | yes, `tests/data`, `tests/phase3/test_source_qualification.py`, `tests/phase3/test_coinbase_wss_qualification.py` | parser/replay fixtures | partial: REST BTC-USD native ticker, Deribit index, and SEC RSS replay passed; Coinbase ETH-USD returned 404 and GDELT returned 429; two real Coinbase Sandbox WSS connections replayed 20 ticker events and 24 heartbeats but observed provider sequence gaps | no continuous source operation | no, reviewed public endpoints only | EXTERNALLY MEASURED / PENDING_EXTERNAL_EVIDENCE | REST evidence root `artifacts/phase3/source-qualification/20260810T041104.946822Z` has 3 passes/2 failures; WSS root `artifacts/phase3/coinbase-wss-qualification/20260810T042758.896119Z` has deterministic replay but sequence gaps on both connections | Preserve both immutable roots; investigate a delivery-guaranteeing reviewed channel/recovery strategy, then collect bounded reconnect/freshness/disagreement evidence without substituting sources |
