...
carried by docs-only main `0fe6865fc2f2ebd9fad2cf6063ba72fb6fba7ec6`.
...
| Phase 4 quantitative baseline council | phase-04 plan | yes; offline paper-utility boundary now requires admitted Phase-3 input, explicit provenance, and closed admission | yes, `tests/models/test_paper_utility.py` plus existing model tests | public-data bake-off, roster, and preparation manifest | no real paper utility | no | no | IMPLEMENTED / TESTED / PENDING_EXTERNAL_EVIDENCE | model role stability is reviewed; admitted Phase 0/3 BTC/ETH paper observations and real net-utility evidence do not exist | Wait for full Phase 0 and Phase 3 admission, then evaluate current candidates against mandatory baselines; do not promote automatically |
