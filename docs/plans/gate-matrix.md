...
| Phase 3 V3-Core source spine | phase-03 plan; real-api-paper-transition.md | yes, including bounded raw-first REST/WSS qualification runners, native event-time normalization, and WSS freshness measurement | yes, `tests/data`, `tests/phase3/test_source_qualification.py`, `tests/phase3/test_coinbase_wss_qualification.py` | parser/replay fixtures | partial: the latest bounded REST retry still records BTC-USD native ticker, Deribit index, and SEC RSS replay passes, Coinbase ETH-USD 404, and GDELT 429; two real Coinbase Sandbox WSS connections replayed 29 ticker events and 23 heartbeats with freshness passing but provider sequence gaps | no continuous source operation | no, reviewed public endpoints only | EXTERNALLY MEASURED / PENDING_EXTERNAL_EVIDENCE | REST evidence root `artifacts/phase3/source-qualification/20260810T044558.818461Z` has 3 passes/2 failures; WSS root `artifacts/phase3/coinbase-wss-qualification/20260810T044142.351959Z` has deterministic replay/freshness but sequence gaps on both connections | Preserve both immutable roots; investigate a delivery-guaranteeing reviewed channel/recovery strategy, then collect bounded freshness/reconnect/disagreement evidence without substituting sources |
