...
| Phase 3 V3-Core source spine | phase-03 plan; real-api-paper-transition.md; formal Phase-3 checklist | yes, including raw-first public BTC/ETH source qualification, typed health/fail-closed selection, lineage, disagreement, replay, and formal checklist/record boundary | yes, Phase-3 parser/replay suites plus `tests/phase3/test_phase3_formal_gate.py` | immutable r7 validator/admission review and broader source pass | r7 structural review and policy review pass; Binance public BTC/ETH is credential-free and write-free, stale intervals fail closed, source identity is preserved, and no silent substitution occurred. Broader pass classifies Coinbase Sandbox ETH-USD HTTP 404 as provider product absence and GDELT HTTP 429 as provider availability/rate limiting; both admit zero observations. Formal checklist has 21 requirements, 16 mandatory evidence items satisfied, and one remaining mandatory predecessor blocker | no new durability window required | no; reviewed public endpoints only | EXTERNALLY MEASURED / PENDING_BASE_GATES | checklist `artifacts/phase3/formal-admission/20260812T004513Z-contract-review/phase3-admission-checklist.json`, SHA-256 `0cd305d79d70a7427100437b977ce028cb643fc885d680a113312b11d3a0a79c`; pending record SHA-256 `0ee4b783c1afa943fb8a9e94ca29ea2c358d6b7e68ba097fd224fd96614d4bbe`; sole blocker `phase_2_formal_predecessor` | supply the legitimate passed Phase-2 predecessor through the dependency-ordered gate registry; then re-evaluate the unchanged immutable evidence; do not launch duplicate r8/r9 evidence |
...
| Phase 4 quantitative baseline council | phase-04 plan | yes; offline paper-utility boundary requires a passed Phase-3 `PhaseGateRecord`, explicit provenance, and closed admission | yes, `tests/models/test_paper_utility.py` plus existing model tests | public-data bake-off, roster, and preparation manifest | no real paper utility | no | no | IMPLEMENTED / TESTED / PENDING_BASE_GATES | selected Phase-0 model roles are independently qualified, but the formal Phase-3 predecessor chain is pending; no real Phase-4 utility evidence exists | Once Phase 3 is formally admitted and the dependency graph confirms no additional global Phase-0 prerequisite, evaluate candidates against mandatory baselines; do not promote automatically |
